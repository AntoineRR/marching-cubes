...
// Position of the center of edges of the cube, (0, 0, 0) being its center
const vec3 edge_position[] = {
	vec3(0.0, -0.5, -0.5),
	vec3(0.5, -0.5, 0.0),
	vec3(0.0, -0.5, 0.5),
	vec3(-0.5, -0.5, 0.0),
...
uint get_noise_bit(uint x, uint y, uint z) {
    ...
    if (density_field_buffer.data[z + y*r + x*r*r] > parameters.iso_level) {
        return 1;
    } else {
        return 0;
    }
...
void main() {
    ...
    // Refer to the cube drawing to understand the corner we choose
    uint cube_index = 0;
    cube_index |= get_noise_bit(id.x, id.y, id.z);
    cube_index |= get_noise_bit(id.x+1, id.y, id.z) << 1;
    cube_index |= get_noise_bit(id.x+1, id.y, id.z+1) << 2;
    cube_index |= get_noise_bit(id.x, id.y, id.z+1) << 3;
    cube_index |= get_noise_bit(id.x, id.y+1, id.z) << 4;
    cube_index |= get_noise_bit(id.x+1, id.y+1, id.z) << 5;
    cube_index |= get_noise_bit(id.x+1, id.y+1, id.z+1) << 6;
    cube_index |= get_noise_bit(id.x, id.y+1, id.z+1) << 7;
    ...
    for (int i = 0; i < index_length; i += 3) {
        Triangle t;
        t.a.xyz = edge_position[triangles[offset + i]] + world_pos;
        t.b.xyz = edge_position[triangles[offset + i + 1]] + world_pos;
        t.c.xyz = edge_position[triangles[offset + i + 2]] + world_pos;
